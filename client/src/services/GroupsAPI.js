...
export const updateDeadline = async (groupId, voting_deadline) => {
  const res = await authFetch(`${BASE_URL}/groups/${groupId}/deadline`, {
    method: 'PUT',
    body: JSON.stringify({ voting_deadline: voting_deadline || null })
  })
  if (!res.ok) {
    const err = await res.json()
